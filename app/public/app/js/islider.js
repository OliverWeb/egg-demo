...
iSlider.prototype._animateFuncs = {
  ...
  default(dom, axis, scale, i, offset) {
    dom.style.webkitTransform = 'translateZ(0) translate' + axis + '(' + (offset + scale * (i - 1)) + 'px)';
  },

  rotate(dom, axis, scale, i, offset) {
    const rotateDirect = (axis == 'X') ? 'Y' : 'X';
    const absoluteOffset = Math.abs(offset);
    const bdColor = window.getComputedStyle(this.wrap.parentNode, null).backgroundColor;
  ...
  depth(dom, axis, scale, i, offset) {
    const rotateDirect = (axis == 'X') ? 'Y' : 'X';
    const zoomScale = (4 - Math.abs(i - 1)) * 0.15;
  ...
  tear(dom, axis, scale, i, offset) {
    const rotateDirect = (axis == 'X') ? 'Y' : 'X';
    const zoomScale = 1 - (Math.abs(i - 1) * 0.2);
...
iSlider.prototype._setUpDamping = function() {
  const oneIn2 = this.scale >> 1;
  const oneIn4 = oneIn2 >> 1;
  const oneIn16 = oneIn4 >> 2;
...
iSlider.prototype._bindHandler = function() {
  ...
  const moveHandler = function(evt) {
    ...
    let offset = evt.targetTouches[0]['page' + axis] - self['start' + axis];
  ...
  const endHandler = function(evt) {
    evt.preventDefault();

    let boundary = self.scale / 2;
    const metric = self.offset;
  ...
  const orientationchangeHandler = function(evt) {
    setTimeout(function() {
      self.reset();
